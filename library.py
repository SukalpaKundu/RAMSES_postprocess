...
class simulation_ramses:
    ...
    def _cellsize(self,snap_id="output_00001"):
        folder  = self.folder+snap_id
        snap    = pb.load(folder)
        DelX    = snap.gas['smooth'].in_units('kpc')
        return DelX
    ...
    def _get_profiles(self,snap_id="output_00001",bins=np.linspace(0,10,1),z_cut=10,r_cut=10,z_cut_profile=10,r_cut_profile=10,component="gas",weight_by="mass",direction="r",verbose=True):
        ...
        if(direction=="r"):
            ...
            if(component=="gas"):
                ...
                VV_p                  = 2*np.pi*(bins+dr_p/2)*(2*z_cut_profile)*dr_p
    ...
    def _get_outflow(self,bins,snap_id="output_00001",z_cut=10,r_cut=10,direction="r"):


        #-----------------usage--------------------

        # vout,Mdot_out = self._get_outflow(bins,snap_id=snap_id,z_cut=z_cut,r_cut=r_cut,direction="r/z")
        ...
        if(direction=="r"):
            ...
            dx                    = self._cellsize(snap_id)
            Mout_r                = np.bincount(id_bin,weights=dd*vr*dx**2,minlength=len(bins))#/(np.bincount(id_bin,weights=wg,minlength=len(bins)))

            scale                 = 0.025   # m_p/cc * km/s * kpc**2 to Msol/yr           
            Mout_r                = scale * Mout_r
        ...
        if(direction=="z"):
            ...
            dx                    = self._cellsize(snap_id)
            Mout_z                = np.bincount(id_bin,weights=dd*vz*dx**2,minlength=len(bins))#/(np.bincount(id_bin,weights=wg,minlength=len(bins)))
    ...
    def _SFR(self,tbin=[],snap_id="output_00001"):
        
        folder   = self.folder+snap_id
        snap     = pb.load(folder)
        
        mstar,tt = np.histogram( tform, bins=tbin, weights = mm )

        sfr      = 1.e-6*mstar/np.diff(tt)
        
        nstar,tt = np.histogram( tform, bins=tbin)

        nsfr     = 1.e-6*nstar/np.diff(tt)
